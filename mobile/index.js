...
//alert(layers.async-pan-zoom.enabled);
...
var images = {
	...
	'pradlo_s' 		:{'url':'pradlo_s3.png','size':4.2},
...
//	'zed_kocka'		:{'url':'kocka.png','size':1.7},
	'kocka2_skok'	:{'url':'bytustka-kocka-sprite-skok.png','size':24.8},
	'kocka2_vis'	:{'url':'bytustka-kocka-sprite-vis.png' ,'size': 8.2},
	'kocka2_splh'	:{'url':'bytustka-kocka-sprite-splh.png','size': 8.1},
	'kocka2_ocas'	:{'url':'bytustka-kocka-sprite-ocas.png','size': 1.0},
...
//	console.log('kocka2_start za 5s');
	setTimeout(kocka2_start,5000);
}
...
function paralax_scroll(){ paralax_timer = null; on_scroll(); }

function scroll_on(){ scroll_run=false; }
...
//	if(Math.round(-1*$('#body_wrapper').position().top)==scroll_top || $('body').hasClass('not_ready')){ scroll_run=false; return; }
//	scroll_top = -1*$('#body_wrapper').position().top;
...
//	}
	...
	var scroll_bottom = Math.round(c_h - h - scroll_top);
	
	//console.log($('#body_wrapper').offset().top+'|'+$('#body_wrapper').position().top);
	...
	if(debug==true) $('#tmp').html('<br />scroll_top = <'+scroll_top+';'+(scroll_top+h)+'><br />scroll_bottom = <'+scroll_bottom+';'+(scroll_bottom+h)+'><br />Y = <'+y+';'+y2+'><br /><br />Ratio:'+(Math.round(w/h*1000)/1000)+'<hr />');	
	...
	$.each(objects,function(id){
		if(y >= this.s0.s && y2 <= this.s1.s){
			...
			if(this.cw > 0){
				var bt = ((this.s1.b-this.s0.b)*cy+this.s0.b)*w + h - scroll_bottom;
//				var bt = ((this.s1.b-this.s0.b)*cy)*w + h;
			}else{
//				$('#'+id).css('bottom',((this.s1.b-this.s0.b)*cy+this.s0.b)*h);
//				$('#'+id).css('bottom',((this.s1.b-this.s0.b)*cy)*h);
				var bt = ((this.s1.b-this.s0.b)*cy+this.s0.b)*h - scroll_bottom;
//				var bt = ((this.s1.b-this.s0.b)*cy)*h;
			}
			$('#'+id).css('bottom',bt);
			...
			if(debug==true) $('#tmp').append('OBJ['+id+']:show ('+Math.round(cy*1000)/1000+') -> '+bt+'<br />');	 //, H='+(this.w / this.cw)+'
		}else{
	...
	// kontrola kocky
	if(y > 0.1 && kocka2_stav==2) kocka2_stav=3; //kocka2_stop();
...
//	return;
	// postranka
	if($('#body_wrapper').hasClass('hidden')){
		scroll_top = Math.round(window.pageYOffset);//Math.round($('#all_wrapper').scrollTop());
...
//		$('#fps').html('UP');
//		var y = Math.floor(Math.max(0,(c_h - h - $('#all_wrapper').scrollTop())/w)* 1000)/1000;
//		var y2 = Math.floor((y - h / w )*1000)/1000; 
		var y = Math.floor(Math.max(0,(c_h - h - scroll_top)/w)* 1000)/1000; //- cor_h//cor_h - 
		var y2 = Math.floor((y - h / w )*1000)/1000;
...
//		alert('loaded');
		//$('#all_wrapper').scroll(function(){
			//console.log('scroll #all_wrapper!');
		//	on_scroll();
		//});
		//$('#all_wrapper').on('scroll',on_scroll);//.scroll(on_scroll);
...
function show_pribeh(){
	...
	body_scroll = Math.round(window.pageYOffset);//scroll_top;//$(document).scrollTop() * 1;
...
function hide_pribeh(anchor){
	var ny = body_scroll;
	var sc_t = Math.round(window.pageYOffset);//$('#all_wrapper').scrollTop();
...
function kocka2_start(){
//	console.log('kocka2_start');
	kocka2_stav = 1;
	kocka2_timer = setInterval(kocka_anim,100);	
/*
	if(kocka2_stav==0){
		kocka2_stav=1;
		$('#kocka2_skok').css('display','block'); 
		setTimeout(function(){ $('#kocka2_start').css('display','none'); },100);	
		setTimeout(function(){ $('#kocka2_skok').css('display','none'); $('#kocka2_vis').css('display','block'); kocka2_stav=2; },1400);
	}*/
}

function kocka_anim(){
	var frame = $('#zed_kocka_2').attr('data-frame')*1+1;
//	console.log('kocka2_anim[stav='+kocka2_stav+',frame='+frame+']');
	if(kocka2_stav==1 && frame > 10) kocka2_stav=2;
	if(kocka2_stav==2 && frame > 20) frame=11;
	if(kocka2_stav==3 && frame > 25){ kocka2_stav=4; }//clearInterval(kocka2_timer); kocka2_timer = setInterval(kocka_anim,200); }
	if(frame > 37) frame=26;
	$('#zed_kocka_2').attr('data-frame',frame);
}

/* function kocka2_stop(){
	if(kocka2_stav==2){
		kocka2_stav=3;
		$('#kocka2_splh').css('display','block'); $('#kocka2_vis').css('display','none');
		$('#kocka2_splh').animate({'bottom':'77vh'},600,function(){
			$('#kocka2_splh').css('display','none'); $('#kocka2_stop').css('display','block');
			kocka2_stav=4;
		});
	}
}*/
...
function show_info(){ $('#page_info').css('opacity',0).removeClass('hidden').stop(true).animate({'opacity':1},500); }

function hide_info(){ $('#page_info').stop(true).animate({'opacity':0},500,function(){ $(this).addClass('hidden'); }); }
...
//			alert(id+':'+this);
	...
	$("#null > img").one("load",function(){ preloaded(this); }).each(function(){ if(this.complete) $(this).load(); });
	...
	$('a[data-href="pribeh"]').click(function(evt){ show_pribeh(); evt.stopPropagation(); evt.preventDefault();	return;	});
	$('#bytustka_link_zpet').click(function(evt){ hide_pribeh(); evt.stopPropagation();	evt.preventDefault(); return; });
